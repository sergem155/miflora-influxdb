...
class ScanDelegate(DefaultDelegate):
	...
	def handleDiscovery(self, dev, isNewDev, isNewData):
		if dev.addr.upper().startswith('C4:7C:8D:'):
			if isNewDev:
				print("Discovered device: %s - %s" % (dev.addr,dev_dict.get(dev.addr.lower())))
			elif isNewData:
				print("Received new data from: %s - %s" % (dev.addr,dev_dict.get(dev.addr.lower())))
...
for dev in device_list:
	if dev.addr.upper().startswith('C4:7C:8D:'):
		hostname = dev_dict.get(dev.addr.lower())
